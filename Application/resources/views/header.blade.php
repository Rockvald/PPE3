        <header>
            <h1>{{ $title_h1 ?? 'CCI' }}</h1>
            {!! Form::open(['url' => 'rechercher', 'id' => 'formrecherche']) !!}
            ...
            <div id="nom_deconnexion">
                <p id="nom_prenom">{{ $donneesPersonnel['Personnel'][0]->prenom }} {{ $donneesPersonnel['Personnel'][0]->nom }}</p>
                <button type="button" name="deconnexion" id="deconnexion" onclick="window.location.href='deconnexion'">Se déconnecter</button>
            </div>
            @if (isset($donneesPersonnel['commande'][0]))
                ...
                @foreach ($donneesPersonnel['commande'] as $lignes => $commande_enCours)
                    <tr>
